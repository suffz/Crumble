...
func RandStringRunes() string {
	...
	return fmt.Sprintf("%v.%v.%v.%v", rand.Intn(300-256)+256, rand.Intn(255), rand.Intn(255), rand.Intn(255))
}
	...
	if utils.Con.NMC.UseNMC {
		...
		if time.Now().Unix() >= utils.Con.NMC.NamemcLoginData.LastAuthed {
			if utils.Con.NMC.Key != "" {
				...
				if _, resp, err := request.Commit(false); err == nil {
			...
			go func() {
				for {
					for _, req := range Reqs {
						CURRSONG = req.Title
						YT, _ := Youtube.Video(req, []string{}, []string{Youtube.AudioLow, Youtube.AudioMedium}, false)
						YT.Play("data/yt/"+req.ID+".mp3", fmt.Sprintf("data/yt/audio_%v_out.mp3", time.Now().UnixNano()), YT.Download())
					}
				}
			}()
		} else {
...
`, StrCmd.String("-name"), namemc.Searches, namemc.HeadURL, namemc.BodyUrl, namemc.Start, namemc.End, namemc.Status)))
					...
					"get-skins": {
						...
						Action: func() {
							...
							for i := 2; i < amt+2; i++ {
								...
								if _, resp, err := request.Commit(false); err == nil && resp.StatusCode == 200 {
					...
					"png": {
						...
						Action: func() {
							...
							if _, resp, err := request.Commit(true); err == nil && resp.StatusCode == 200 {
