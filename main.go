...
`, StrCmd.String("-name"), namemc.Searches, namemc.HeadURL, namemc.BodyUrl, namemc.Start, namemc.End, namemc.Status)))
						...
						Action: func() {
							...
							for i := 2; i < amt+2; i++ {
								request, _ := h2.BuildRequest("https://namemc.com/minecraft-skins"+Path, "GET", "", followbot.CommonHeaders,
									nil, followbot.ReturnCF())

								if _, resp, err := request.Commit(); err == nil && resp.StatusCode == 200 {
									Skins = append(Skins, followbot.GetAllSkinsOnPage(string(resp.Body))...)
							...
							fmt.Println(utils.Logo(fmt.Sprintf(`:> Skins added to %v!`, path)))
						},
					},
					"follow": {
						...
						Action: func() {
							if utils.Con.CF.Tokens == "" {
								fmt.Println(utils.Logo("Error: namemc clearance undetected."))
								return
							}
							if utils.Con.NMC.NamemcLoginData.Token == "" {
								fmt.Println(utils.Logo("Error: namemc token isnt valid or null."))
								return
							}
					...
					"rl-test": {
						...
						Action: func() {
							...
							var Info []Resps
							request, _ := h2.BuildRequest("https://namemc.com/minecraft-names", "GET", "", followbot.CommonHeaders, nil, followbot.ReturnCF())
						Exit:
							for {
								var I []Resps
								if _, resp, err := request.Commit(); err == nil {
									if resp.StatusCode == 200 {
										if doc, err := html.Parse(strings.NewReader(string(resp.Body))); err == nil {
											for i := 1; i <= 119; i += 2 {
												var Searches string
												...
												if start != nil && end != nil {
													if len(start.Attr) > 0 && len(end.Attr) > 0 {
														...
														name := ""
														if html.FindOne(doc, fmt.Sprintf("/html/body/main/div/div[4]/div/table/tbody/tr[%v]/td[1]/a", i)) != nil && html.FindOne(doc, fmt.Sprintf("/html/body/main/div/div[4]/div/table/tbody/tr[%v]/td[1]/a", i)).FirstChild != nil {
															name = html.FindOne(doc, fmt.Sprintf("/html/body/main/div/div[4]/div/table/tbody/tr[%v]/td[1]/a", i)).FirstChild.Data
														}
														I = append(I, Resps{
					...
					"png": {
						...
						Action: func() {
							...
							if _, resp, err := h2.RClient().Redirect(&request); err == nil {
								resp = request.Redirects
								if !strings.Contains(resp.Request.URL.String(), "search") {
									img := followbot.GenImages(string(resp.Body))
									path := "data/skinarts/" + strings.ReplaceAll(uuid.NewString(), "-", "")[0:4] + "_" + name + ".png"
								...
								fmt.Println(resp)
								if err != nil {
									fmt.Println(utils.Logo(fmt.Sprintf("<%v> [ERROR] %v Unsatisfactory Status Code From namemc.com!", time.Now().Format("05.000"), err.Error())))
								} else {
									fmt.Println(utils.Logo(fmt.Sprintf("<%v> [ERROR] %v Unsatisfactory Status Code From namemc.com!", time.Now().Format("05.000"), resp.StatusCode)))
								}
					...
					"3c": {
						...
						Action: func() {
							...
							for _, names := range Use {
								fmt.Println(utils.Logo(fmt.Sprintf(`%v %v-%v`, names.Name, names.Start_Unix, names.End_Unix)))
							}
						},
					},
					"mass-follow": {
						...
						Action: func() {
							...
							h2.RClient().Redirect(&request)
							request.ChangeURL(request.Request.URL.String() + "/followers")
							...
							for {
								if _, resp, err := request.Commit(); err == nil {
									...
									if !strings.Contains(string(body), `rel="next" title="Next Page"`) {
										...
										request.ChangeURL("https://" + request.Request.Host + request.Request.URL.Path + next + "?sort=date:desc&page=" + strings.Split(strings.Split(d[len(d)-1], `?sort=date:desc&amp;page=`)[1], `"`)[0])
									}
									...
									for _, data := range bodys {
										if strings.Contains(data, "/profile/") {
											...
											ID := strings.Split(strings.Split(data, `?id=`)[1], `&`)[0]
			...
			"snipe": {
				...
				Action: func() {
					if len(utils.Bearer.Details) != 0 {
						...
						logger.TimeFormat = "15:04:05.0000"
						...
						if time.Now().Unix() < drop.Unix() {
							...
							fmt.Println(utils.Logo("Duration Until "+name+" Drops: "+time.Until(drop).String()), "\n")
							prog, _ := pterm.DefaultProgressbar.WithTotal(int(time.Until(drop).Seconds())).WithTitle(fmt.Sprintf("[%v] Waiting...", name)).Start()
										...
										go func(Config apiGO.Info, c Proxys) {
											...
											if P, ok, _ := utils.Connect(c.Proxy); ok {
												for i := 0; i < map[string]int{"Giftcard": utils.Con.Settings.GC_ReqAmt, "Microsoft": utils.Con.Settings.MFA_ReqAmt}[Config.AccountType]; i++ {
													...
													go func() {
														...
														case "200":
															...
															logger.Info(fmt.Sprintf(`[%v] %v`,
																...
																"Sent", Req.ResponseDetails.SentAt.Format("15:04:05.0000"),
																"Recv", time.Now().Format("15:04:05.0000"),
																"NameMC", NMC,
																"Email", utils.HashEmailClean(Config.Email),
															))
														case "400", "403":
															logger.Info(fmt.Sprintf(`[%v] %v`,
																Req.ResponseDetails.StatusCode,
																name,
															), logger.Args(
																"Sent", Req.ResponseDetails.SentAt.Format("15:04:05.0000"),
																"Recv", time.Now().Format("15:04:05.0000"),
														...
														case "429":
															logger.Error(fmt.Sprintf(`[%v] %v`,
																...
																"Error", "RATE LIMITED",
																"Status", Req.ResponseDetails.StatusCode,
																"Email", utils.HashEmailClean(Config.Email),
															))
														case "401":
															logger.Error(fmt.Sprintf(`[%v] %v`,
													...
													time.Sleep(TempCalc(map[string]int{"Giftcard": utils.Con.Settings.SleepAmtPerGc, "Microsoft": utils.Con.Settings.SleepAmtPerMfa}[Config.AccountType], utils.Accamt*2))
												}
											}
											wg.Wait()
										}(accs, c)
						...
						for {
							if cl || Changed {
								fmt.Println()
								if EmailClaimed == "" {
									logger.Error("No account has sniped the name.")
									return
								}
								logger.Error(EmailClaimed)
								return
	...
	if utils.Con.NMC.UseNMC {
		...
		if time.Now().Unix() >= utils.Con.NMC.NamemcLoginData.LastAuthed {
			if utils.Con.NMC.Key != "" {
				...
				Headers["content-length"] = strconv.Itoa(len(fmt.Sprintf(`email=%v&password=%v`, acc[0], acc[1])))
				request, _ := h2.BuildRequest("https://namemc.com/login", "POST", fmt.Sprintf(`email=%v&password=%v`, acc[0], acc[1]), Headers, nil, followbot.ReturnCF())
				if _, resp, err := request.Commit(); err == nil {
