					...
					if len(utils.Bearer.Details) != 0 {
						...
						go func() {
							...
							for _, Acc := range utils.Accs["Giftcard"] {
								...
								ip, port, user, pass := utils.GetProxyStrings(Acc.Proxy)
								started := time.Now()

								for _, data := range Acc.Accs {
									D = append(D, AccData{Request: utils.ReturnRequest(data.AccountType, data.Bearer, name, &h2.ProxyAuth{IP: ip, Port: port, User: user, Password: pass}), Accounts: data})
								}

								logger.Info(fmt.Sprintf(`Set Up Proxy(s) <accs:%v>:`, len(Acc.Accs)), logger.Args(
									"IP", ip,
									"PORT", port,
									"USER", user,
									"STARTED", started.Format("15:04:05.0000"),
									"ENDED", time.Now().Format("15:04:05.0000"),
								))
							...
							for !cl || !Changed {
								for _, c := range Payloads {
									for _, accs := range c.Accounts {
										go func(Config AccData, c Proxys) {
											var wg sync.WaitGroup
											for i := 0; i < map[string]int{"Giftcard": utils.Con.Settings.GC_ReqAmt, "Microsoft": utils.Con.Settings.MFA_ReqAmt}[Config.Accounts.AccountType]; i++ {
												wg.Add(1)
												go func() {

													sent := time.Now()

													if _, resp, err := Config.Request.Commit(); err == nil {
														switch resp.StatusCode {
														case 200:
															if utils.Con.SkinChange.Link != "" {
																apiGO.ChangeSkin(apiGO.JsonValue(utils.Con.SkinChange), Config.Accounts.Bearer)
															}
															NMC := utils.Namemc_key(Config.Accounts.Bearer)
															if utils.Con.NMC.UseNMC {
															...
															logger.Info(fmt.Sprintf(`[%v] %v`,
																...
																"Sent", sent.Format("15:04:05.0000"),
																"Recv", time.Now().Format("15:04:05.0000"),
																"Length", resp.ContentLength,
																"NameMC", NMC,
																"Email", utils.HashEmailClean(Config.Accounts.Email),
															))
