...
func Namemc_key(bearer string) string {
	...
	player = basic.NewPlayer(client, basic.DefaultSettings, basic.EventsListener{
		SystemMsg: func(c chat.Message, overlay bool) error {
			if strings.Contains(c.ClearString(), Info_Acc.Name+" joined the game.") {
				if err := chatHandler.SendMessage("/namemc"); err != nil {
					client.Close()
					return err
				}
			} else if strings.Contains(c.ClearString(), "https://namemc.com/claim?key=") {
				key := c.ClearString()
				client.Close()
		...
		GameStart:    func() error { return nil },
		Disconnect:   func(reason chat.Message) error { return errors.New(reason.ClearString()) },
		HealthChange: func(health float32) error { return nil },
		Death: func() error {
			go func() {
	...
	chatHandler = msg.New(client, player, msg.EventsHandler{
		PlayerChatMessage: func(msg chat.Message) error { return nil },
	})
