...
func Swapprofile(Profile string) {
	...
	request, _ := h2.BuildRequest(Profile, "GET", "", Headers,
		ReturnCF(), &http.Cookie{Name: "session_id", Value: Session.Key}, &http.Cookie{Name: "referrer", Value: "https://namemc.com/"})
	request.Commit(nil)
}
...
func Claim_NAMEMC(url string) {
	...
	request, _ := h2.BuildRequest(url, "GET", "", Headers,
		ReturnCF(), &http.Cookie{Name: "session_id", Value: Session.Key}, &http.Cookie{Name: "referrer", Value: url})
	request.Commit(nil)
}
...
func (Data *Target) Send(precheck bool) bool {
	...
	request_get, _ := h2.BuildRequest(Data.URL, "GET", "", CommonHeaders, ReturnCF(), &http.Cookie{Name: "session_id", Value: Session.Key})
	request_post, _ := h2.BuildRequest(Data.URL, "POST", fmt.Sprintf(`profile=%v&task=follow`, Data.UUID), Headers, ReturnCF(), &http.Cookie{Name: "session_id", Value: Session.Key}, &http.Cookie{Name: "referrer", Value: Data.URL})

	var cont bool
	var nn string
	if precheck {
		if _, resp, err := request_get.Commit(nil); err == nil && resp.StatusCode == 200 {
	...
	if !cont {
		if _, resp, err := request_post.Commit(nil); err == nil {
			switch resp.StatusCode {
			case 302:
				if _, resp, err := request_get.Commit(nil); err == nil {
...
func GetProfiles() {
	request, _ := h2.BuildRequest("https://namemc.com/", "GET", "", CommonHeaders,
		ReturnCF(), &http.Cookie{Name: "session_id", Value: Session.Key})
...
func SendFollow(name string) bool {
	...
	request_get, _ := h2.BuildRequest("https://namemc.com/profile/"+name, "GET", "", CommonHeaders, ReturnCF())
	var redirect string
	if _, resp, err := request_get.Commit(nil); err == nil {
		request_get, _ = h2.BuildRequest("https://namemc.com"+resp.Header.Get("Location"), "GET", "", CommonHeaders, ReturnCF(), &http.Cookie{Name: "session_id", Value: Session.Key})
		redirect = "https://namemc.com" + resp.Header.Get("Location")
	}
	if _, resp, err := request_get.Commit(nil); err == nil && resp.StatusCode == 200 {
		if body, err := io.ReadAll(resp.Body); err == nil {
			...
			Headers["content-type"] = "application/x-www-form-urlencoded"
			Headers["origin"] = "https://namemc.com"
			Headers["referer"] = redirect
			request_post, _ := h2.BuildRequest(redirect, "POST", fmt.Sprintf(`profile=%v&task=follow`, strings.Split(strings.Split(string(body), `order-lg-2 col-lg" style="font-size: 90%"><samp>`)[1], `</samp></div>`)[0]), Headers, ReturnCF(), &http.Cookie{Name: "session_id", Value: Session.Key}, &http.Cookie{Name: "referrer", Value: redirect})
			if _, resp, err := request_post.Commit(nil); err == nil {
