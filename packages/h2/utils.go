...
func (R *Request) Commit() (_a *tls.UConn, Resp Response_R, _e error) {
		...
		for s.Scan() {
			...
			if found_start_of_data {
				...
				case 300, 301, 302, 303, 307, 308:
					Resp.Body = bytes.NewBuffer([]byte(resp_for_body))
					Resp.data_for_decoding = []byte(respa)
					break Exit
				}
			}
			if lines_content_length == 0 && strings.Contains(s.Text(), "</html>") {
				Resp.Body = bytes.NewBuffer([]byte(resp_for_body))
				Resp.data_for_decoding = []byte(respa)
				break
			} else if lines_content_length == 0 && s.Text() == "}" {
				Resp.Body = bytes.NewBuffer([]byte(resp_for_body))
				Resp.data_for_decoding = []byte(respa)
				break
			} else {
...
type ClientR struct {
	Redirect func(req *Request) (_a *tls.UConn, Resp Response_R, _e error)
}

func RClient() ClientR {
	return ClientR{Redirect: func(req *Request) (_a *tls.UConn, Resp Response_R, _e error) {
