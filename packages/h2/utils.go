...
func (R *Request) Commit(P *ProxyAuth) (*tls.UConn, *http.Response, error) {
	var conn *tls.UConn
...
func BuildRequest(URL, method, data string, headers map[string]string, cookies ...*http.Cookie) (Request, error) {
	u, err := url.Parse(URL)
	if err != nil {
		return Request{}, err
	}
