...
func BuxDroptimes(name string) (int64, int64) {
	var data NameRequest
	resp, _ := http.Get("https://buxflip.com/data/info/" + name)
	b, _ := io.ReadAll(resp.Body)
	json.Unmarshal(b, &data)
	if !reflect.DeepEqual(data, NameRequest{}) {
		return data.Start, data.End
	}
	return 0, 0
}

func StarShoppingDroptime(name string) (int64, int64) {
	...
	dropTimeBytes, _ := io.ReadAll(data.Body)
	var f Payload
	json.Unmarshal(dropTimeBytes, &f)
	return f.Start, f.End
}

func DropTime(name string) (int64, int64) {
	if Start, End := BuxDroptimes(name); Start > 0 && End > 0 {
		return Start, End
	}
	if Start, End := StarShoppingDroptime(name); Start > 0 && End > 0 {
		return Start, End
	}
	return 0, 0
}
...
func CheckChange(bearer string, PS *h2.ProxyAuth) bool {
	...
	var err error
	if PS != nil {
		Conn, err = (&h2.Client{Config: h2.GetDefaultConfig()}).Connect("https://api.minecraftservices.com/minecraft/profile/namechange", h2.ReqConfig{ID: 1, BuildID: tls2.HelloChrome_106_Shuffle, DataBodyMaxLength: 1609382, Proxy: PS})
	} else {
		Conn, err = (&h2.Client{Config: h2.GetDefaultConfig()}).Connect("https://api.minecraftservices.com/minecraft/profile/namechange", h2.ReqConfig{ID: 1, BuildID: tls2.HelloChrome_106_Shuffle, DataBodyMaxLength: 1609382})
	}
	if err != nil {
		return false
	} else {
...
func Search(username string) (data Bux) {
	if req, _ := http.Get("https://buxflip.com/data/search/" + username); req.StatusCode < 300 {
		json.Unmarshal([]byte(ReturnJustString(io.ReadAll(req.Body))), &data)
	}
	return
}
...
func ThreeLetters(option string) (Data []NameRequest) {
	var data Bux2
	switch option {
	case "3c":
